...
void RaiseError_NestedTemplateClass(IdentifyNode* node)
{
	...
	sprintf(buf, "\'%s\' : template class can not as a nested type", node->m_str.c_str());
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : is not a type name", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
void RaiseError_InvalidTypeName(ScopeNameListNode* node)
{
	...
	sprintf(buf, "\'%s\' : is not a type name", str.c_str());
...
void RaiseError_InvalidParameterType(ParameterNode* node)
{
	...
	sprintf(buf, "\'%s %s %s\' : is not a valid type as paramter", str.c_str(), strOut, strPassing);
...
void RaiseError_InvalidResultType(TypeNameNode* result, TokenNode* passing, bool resultArray)
{
	...
	sprintf(buf, "\'%s %s\' : can not be a result type", str.c_str(), strPassing);
	ErrorList_AddItem_CurrentFile(tokenNode->m_lineNo,
...
void RaiseError_InvalidFieldType(FieldNode* node)
{
	...
	sprintf(buf, "\'%s\' : can not be a field type", str.c_str());
	ErrorList_AddItem_CurrentFile(tokenNode->m_lineNo,
...
void RaiseError_InvalidPropertyType(PropertyNode* node)
{
	...
	TokenNode* tokenNode = node->m_typeName->m_scopeNameList ? node->m_typeName->m_scopeNameList->m_scopeName->m_name : node->m_typeName->m_keyword;
	sprintf(buf, "\'%s %s\' : can not be a property type", str.c_str(), strPassing);
	ErrorList_AddItem_CurrentFile(tokenNode->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : is not a class template name", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
void RaiseError_InvalidClassTemplateName(TypeNameNode* node)
{
	...
	node->getString(name);
	IdentifyNode* identifyNode = node->m_scopeNameList->m_scopeName->m_name;
	sprintf(buf, "\'%s\' : is not a class template name", name.c_str());
	ErrorList_AddItem_CurrentFile(identifyNode->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : too many template arguments", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : too few template arguments", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
void RaiseError_InvalidTemplateArgument(TypeNameNode* node)
{
	...
	std::string name;
	node->getString(name);
	sprintf(buf, "\'%s\' : is not a valid template argument", name.c_str());
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "redefinition of template parameter \'%s\'", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : template class is not explicit instantition", typeName);
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : template interface is not supported", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : reference type must inherited from ::pafcore::Reference", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
{
	char buf[error_info_buffer_size];
	sprintf(buf, "\'%s\' : override method must be virtual or abstract", node->m_str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_lineNo,
...
void RaiseError_TooFewFormalParameters(OperatorNode* node)
{
	...
	sprintf(buf, "\'operator %s\' : too few formal parameters", str.c_str());
...
void RaiseError_TooManyFormalParameters(OperatorNode* node)
{
	...
	std::string str;
	node->getOperatorString(str);
	sprintf(buf, "\'operator %s\' : too many formal parameters", str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_sign->m_lineNo,
...
void RaiseError_StaticOperator(OperatorNode* node)
{
	...
	std::string str;
	node->getOperatorString(str);
	sprintf(buf, "\'operator %s\' : static operator overloading is not support", str.c_str());
	ErrorList_AddItem_CurrentFile(node->m_sign->m_lineNo,
