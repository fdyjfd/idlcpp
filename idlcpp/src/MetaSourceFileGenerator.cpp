...
void MetaSourceFileGenerator::generateCode_Program(FILE* file, ProgramNode* programNode, const char* fileName, const char* cppName)
{
	...
	writeStringToFile(buf, file);
	sprintf_s(buf, "#include \"%sNullType.h\"\n", pafcorePath.c_str());
	writeStringToFile(buf, file);
	writeStringToFile("\n", file);
...
void MetaSourceFileGenerator::generateCode_Enum(FILE* file, EnumNode* enumNode, int indentation)
{
	...
	//enumNode->getFullName(typeFullName, 0);
	//typeFullNames->
...
void writeOverrideFunction(ClassNode* classNode, TemplateArgumentMap* templateArguments, FILE* file, int indentation)
{
	char buf[512];
	...
	FILE* file, int indentation)
{
	...
	writeMetaConstructor_Member(nestedTypeNodes,
		staticFieldNodes, staticPropertyNodes, staticMethodNodes,
...
{
	assert(!methodNode->m_result->isVoid());	
	char buf[512];
	std::string typeName;
...
void writeInterfaceMethodImpl_CallBaseClass(ClassNode* classNode, MethodNode* methodNode, std::vector<ParameterNode*>& parameterNodes, FILE* file, int indentation)
{
	...
	bool isVoid = methodNode->m_result->isVoid();
