...
class PAFSTD_TEMPLATE UniqueOrderedArray : private std::vector<T, A>
{
public:
	typedef T key_type;
...
public:
	...
	{
		return vector_type::get_allocator();
	}
	...
	{
		PAF_ASSERT(empty() || (end() == it && key_compare()(*(it - 1), x)) || key_compare()(x, *it));
